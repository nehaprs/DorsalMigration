...
setwd("~/BINF/scrnaseq general/dorsal migration/full head/highUMI/integrated/all4/diffexp/filt round1")
library(readxl)
df <- read_excel("s17d_vs_17v.xlsx")
df2 = df[,c(2,5,6)]
...
writexl::write_xlsx( as.data.frame(gsea_reactome),"s17d_vs_17v-gsea_reactome.xlsx" )


p = dotplot(gsea_reactome, showCategory = 20) +
  ggtitle("Reactome GSEA")

ggsave("s17d_vs_17v-gsea_reactome.png", plot = p, width = 10, height = 12)
